    ...
    public sealed class LavaLinkAudio
    {
        ...
        public async Task<Embed> LeaveAsync(IGuild guild)
        {
            try
            {
                ...
                return await EmbedHandler.CreateBasicEmbed("Music", $"I've left. Thank you for playing Ununin.");
            }
        ...
        public async Task<Embed> ListAsync(IGuild guild)
        {
            try
            {
                ...
                if (player.PlayerState is PlayerState.Playing)
                {
                    ...
                    {
                        return await EmbedHandler.CreateBasicEmbed($"Now Playing: {player.Track.Title}", "Nothing Else Is Queued.");
                    }
                    else
                    {
        ...
        public async Task TrackEnded(TrackEndedEventArgs args)
        {
            ...
            await args.Player.TextChannel.SendMessageAsync(
                embed: await EmbedHandler.CreateBasicEmbed("Now Playing", $"[{track.Title}]({track.Url})"));
        }
